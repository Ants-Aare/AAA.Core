...
namespace AAA.Core.Runtime.Singleton
{
    ...
    namespace AAA.Core.Runtime.Singleton
    {
        public class GenericSingletonWithDummy<T, TDummy> : MonoBehaviour
            where T : Component
            where TDummy : T
        {
            protected static T instance;
            public static T Instance
            {
                get
                {
                    ...
                    var obj = new GameObject();
                    obj.name = $"{typeof(TDummy).Name} -AUTOCREATED-";
                    instance = obj.AddComponent<TDummy>();
                    (instance as GenericSingletonWithDummy<T, TDummy>).InitializeSingleton();
                    Debug.LogError($"A Singleton of type {instance.GetType().ToString()} was not found. It has been automatically created." +
                                     "You might want to consider having one in the Scene by default.");
                    return instance; 
                }
            }
